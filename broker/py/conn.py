...
def connect() -> None:
    # init
    broker_dealer = app.state().c.socket(zmq.DEALER)
    broker_sub = app.state().c.socket(zmq.SUB)
    broker_sub.setsockopt(zmq.SUBSCRIBE, b"")
    in_router = app.state().c.socket(zmq.ROUTER)
    worker_router = app.state().c.socket(zmq.ROUTER)
    # poll
    app.state().poller.register(broker_sub, zmq.POLLIN)
    app.state().poller.register(in_router, zmq.POLLIN)
    app.state().poller.register(worker_router, zmq.POLLIN)
    # connect
    _connect_socket(
        broker_dealer,
